#!/usr/bin/env python3
"""
Eye-to-Hand 手眼标定完整 Demo
===============================

场景: 相机固定在世界坐标系中 (眼在手外), 标定板安装在机械臂末端。
...
流程概览:
    ...
    2. 用 Tsai / Navy / Park 三种方法分别求解 X = T_cam^base
    3. 与真值对比, 计算旋转误差和平移误差
    4. 输出对比表格
...
def main():
    print("=" * 60)
    print("  Eye-to-Hand 手眼标定 Demo")
    print("  相机固定 (眼在手外), 标定板在机械臂上")
    print("=" * 60)
    ...
    # ============================================================
    print("\n[1/4] 生成合成标定数据 ...")
    n_poses = 15  # eye-to-hand 建议更多姿态以提高精度
    noise_rot = 0.15
    noise_trans = 0.003

    data: CalibrationData = generate_eye_to_hand_data(
        n_poses=n_poses,
    ...
    errors = {"Tsai-Lenz": r_tsai, "Navy": r_navy, "Park": r_park}
    ...
    X_unified = solve_hand_eye(A, B, method="navy")
    plot_calibration_error(
        A_list=data.A_list,
        B_list=data.B_list,
        X_est=X_unified,
        title=f"Eye-to-Hand 标定残差 (Navy, 旋转误差={r_navy:.4f}°)",
        save_path=str(output_dir / "eye_to_hand_error.png"),
    )
    print(f"  残差图已保存: {output_dir / 'eye_to_hand_error.png'}")
